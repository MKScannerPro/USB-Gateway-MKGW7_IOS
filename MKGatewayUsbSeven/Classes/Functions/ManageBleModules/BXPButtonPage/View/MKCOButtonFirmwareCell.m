...
@implementation MKCOButtonFirmwareCellModel
@end

@interface MKCOButtonFirmwareCell ()
...
@implementation MKCOButtonFirmwareCell

+ (MKCOButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKCOButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCOButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKCOButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCOButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(co_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate co_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCOButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOButtonFirmwareCellModel.class]) {
        return;
    }
