...
@interface MKCOMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCOMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCOMqttServerModel *dataModel;

@property (nonatomic, strong)MKCOMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCOMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCOMqttServerController

- (void)dealloc {
    NSLog(@"MKCOMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCOReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKCOMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)co_selectedServerParams:(NSString *)fileName {
    [MKCOExcelDataManager parseDeviceExcel:fileName
        ...
        MKCOMqttServerModel *model = [MKCOMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCOMqttServerModel *model = [[MKCOMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOMqttServerController", @"co_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCOMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCOMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCOMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCOMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCOMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCOMqttServerConfigFooterViewModel alloc] init];
    }
