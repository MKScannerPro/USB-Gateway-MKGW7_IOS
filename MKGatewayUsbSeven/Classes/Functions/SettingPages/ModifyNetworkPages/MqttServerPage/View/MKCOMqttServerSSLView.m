...
@implementation MKCOMqttServerSSLViewModel
@end

@interface MKCOMqttServerSSLView ()<MKCOMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCOMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCOMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCOMqttServerSSLTextField *clientCertView;

@end

@implementation MKCOMqttServerSSLView
...
#pragma mark - MKCOMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)co_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate co_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate co_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate co_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCOMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCOMqttServerSSLTextFieldModel *caModel = [[MKCOMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCOMqttServerSSLTextFieldModel *clientKeyModel = [[MKCOMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCOMqttServerSSLTextFieldModel *clientModel = [[MKCOMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayUsbSeven", @"MKCOMqttServerSSLView", @"co_switchSelectedIcon.png") : LOADICON(@"MKGatewayUsbSeven", @"MKCOMqttServerSSLView", @"co_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOMqttServerSSLView", @"co_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCOMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCOMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCOMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCOMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCOMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCOMqttServerSSLTextField alloc] init];
