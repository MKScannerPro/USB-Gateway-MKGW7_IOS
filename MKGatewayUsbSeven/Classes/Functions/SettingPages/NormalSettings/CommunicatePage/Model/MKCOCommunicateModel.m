...
@interface MKCOCommunicateModel ()
...
@implementation MKCOCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKCOMQTTInterface co_readCommunicateTimeoutWithMacAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKCOMQTTInterface co_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
