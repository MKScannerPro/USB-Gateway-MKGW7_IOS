...
@interface MKCOSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCOSystemTimeCellDelegate,
...
@implementation MKCOSystemTimeController

- (void)dealloc {
    NSLog(@"MKCOSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCONTPServerController *vc = [[MKCONTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCOSystemTimeCell *cell = [MKCOSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCOSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCOMQTTInterface co_readDeviceUTCTimeWithMacAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCOMQTTInterface co_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCOSystemTimeCellModel *cellModel = [[MKCOSystemTimeCellModel alloc] init];
    cellModel.index = 0;
