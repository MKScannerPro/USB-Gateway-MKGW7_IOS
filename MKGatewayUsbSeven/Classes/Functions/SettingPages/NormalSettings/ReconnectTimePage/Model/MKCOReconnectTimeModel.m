...
@interface MKCOReconnectTimeModel ()
...
@implementation MKCOReconnectTimeModel
...
- (BOOL)readReconnectTimeout {
    ...
    [MKCOMQTTInterface co_readNetworkReconnectTimeoutWithMacAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReconnectTimeout {
    __block BOOL success = NO;
    [MKCOMQTTInterface co_configReconnectTimeout:[self.timeout integerValue] macAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
