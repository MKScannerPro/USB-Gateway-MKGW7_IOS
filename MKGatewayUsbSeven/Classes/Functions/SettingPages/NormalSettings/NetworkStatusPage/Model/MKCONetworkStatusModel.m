...
@interface MKCONetworkStatusModel ()
...
@implementation MKCONetworkStatusModel
...
- (BOOL)readReportInterval {
    ...
    [MKCOMQTTInterface co_readNetworkStatusReportIntervalWithMacAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportInterval {
    __block BOOL success = NO;
    [MKCOMQTTInterface co_configNetworkStatusReportInterval:[self.interval integerValue] macAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
