...
@implementation MKCOMQTTSSLForAppViewModel
@end

@interface MKCOMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCOMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate co_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate co_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate co_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate co_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCOMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTSSLForAppView", @"co_switchSelectedIcon.png") : LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTSSLForAppView", @"co_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTSSLForAppView", @"co_switchUnselectedIcon.png") forState:UIControlStateNormal];
