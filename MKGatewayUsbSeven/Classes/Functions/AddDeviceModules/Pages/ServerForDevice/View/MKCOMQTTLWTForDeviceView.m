...
@implementation MKCOMQTTLWTForDeviceViewModel
@end

@interface MKCOMQTTLWTForDeviceView ()
...
@implementation MKCOMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTLWTForDeviceView", @"co_switchSelectedIcon.png") : LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTLWTForDeviceView", @"co_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(co_lwt_statusChanged:)]) {
        [self.delegate co_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTLWTForDeviceView", @"co_switchSelectedIcon.png") : LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTLWTForDeviceView", @"co_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(co_lwt_retainChanged:)]) {
        [self.delegate co_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(co_lwt_qosChanged:)]) {
        [self.delegate co_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCOMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTLWTForDeviceView", @"co_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTLWTForDeviceView", @"co_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(co_lwt_topicChanged:)]) {
                [self.delegate co_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(co_lwt_payloadChanged:)]) {
                [self.delegate co_lwt_payloadChanged:text];
            }
        };
