//
//  MKCOServerConfigDeviceSettingView.m
...
@interface MKCOServerConfigDeviceSettingView ()
...
@implementation MKCOServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate co_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate co_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate co_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
