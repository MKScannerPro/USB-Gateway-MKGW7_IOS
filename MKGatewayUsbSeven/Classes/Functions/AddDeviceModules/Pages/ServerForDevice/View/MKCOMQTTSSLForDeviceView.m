...
@implementation MKCOMQTTSSLForDeviceViewModel
@end

@interface MKCOMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCOMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate co_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate co_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate co_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate co_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(co_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate co_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCOMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTSSLForDeviceView", @"co_switchSelectedIcon.png") : LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTSSLForDeviceView", @"co_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOMQTTSSLForDeviceView", @"co_switchUnselectedIcon.png") forState:UIControlStateNormal];
