//
//  MKCOServerConfigDeviceFooterView.m
...
@implementation MKCOServerConfigDeviceFooterViewModel
@end
...
@interface MKCOServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCOMQTTSSLForDeviceViewDelegate,
MKCOServerConfigDeviceSettingViewDelegate,
MKCOMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKCOMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKCOMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKCOServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCOServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate co_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate co_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate co_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate co_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate co_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCOMQTTSSLForDeviceViewDelegate
- (void)co_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate co_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)co_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate co_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)co_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate co_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)co_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate co_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)co_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate co_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKCOServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)co_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate co_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKCOMQTTLWTForDeviceViewDelegate
- (void)co_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate co_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)co_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate co_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)co_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate co_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)co_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate co_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)co_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(co_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate co_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCOServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKCOMQTTSSLForDeviceViewModel *sslModel = [[MKCOMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKCOMQTTLWTForDeviceViewModel *lwtModel = [[MKCOMQTTLWTForDeviceViewModel alloc] init];
...
- (MKCOMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCOMQTTSSLForDeviceView alloc] init];
...
- (MKCOMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKCOMQTTLWTForDeviceView alloc] init];
...
- (MKCOServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKCOServerConfigDeviceSettingView alloc] init];
