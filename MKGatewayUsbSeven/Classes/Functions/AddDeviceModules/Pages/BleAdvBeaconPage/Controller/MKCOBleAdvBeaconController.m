...
@interface MKCOBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCOBleAdvBeaconModel *dataModel;

@end

@implementation MKCOBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCOBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOBleAdvBeaconController", @"co_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCOBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCOBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
