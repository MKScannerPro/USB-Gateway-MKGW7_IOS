...
- (void)syncButtonPressed {
    ...
    if (ValidStr([MKCOUserLoginManager shared].password)) {
        //已经登陆过
        [self login:[MKCOUserLoginManager shared].isHome username:[MKCOUserLoginManager shared].username password:[MKCOUserLoginManager shared].password];
        return;
    }
    MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
    viewModel.account = [MKCOUserLoginManager shared].username;
    viewModel.isHome = [MKCOUserLoginManager shared].isHome;
    viewModel.password = [MKCOUserLoginManager shared].password;
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKNormalService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [[MKCOUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
        MKCOSyncDeviceController *vc = [[MKCOSyncDeviceController alloc] init];
