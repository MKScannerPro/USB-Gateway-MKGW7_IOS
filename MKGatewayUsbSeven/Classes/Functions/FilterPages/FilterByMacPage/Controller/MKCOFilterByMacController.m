...
@interface MKCOFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCOFilterByMacModel *dataModel;

@end

@implementation MKCOFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOFilterByMacController", @"co_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCOFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCOFilterByMacModel alloc] init];
    }
    return _dataModel;
}
