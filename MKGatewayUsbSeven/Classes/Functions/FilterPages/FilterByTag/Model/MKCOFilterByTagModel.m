...
@interface MKCOFilterByTagModel ()
...
@implementation MKCOFilterByTagModel
...
- (BOOL)readFilterByTag {
    ...
    [MKCOMQTTInterface co_readFilterBXPTagWithMacAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTag:(NSArray <NSString *>*)tagList {
    __block BOOL success = NO;
    [MKCOMQTTInterface co_configFilterByTag:self.isOn preciseMatch:self.precise reverseFilter:self.reverse tagIDList:tagList macAddress:[MKCODeviceModeManager shared].macAddress topic:[MKCODeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
