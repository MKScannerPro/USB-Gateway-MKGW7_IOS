//
//  MKCOFilterByAdvNameController.m
...
@interface MKCOFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCOFilterByAdvNameModel *dataModel;

@end

@implementation MKCOFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKCOFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOFilterByAdvNameController", @"co_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCOFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCOFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
