...
@interface MKCOFilterByURLController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCOFilterByURLModel *dataModel;

@end

@implementation MKCOFilterByURLController

- (void)dealloc {
    NSLog(@"MKCOFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayUsbSeven", @"MKCOFilterByURLController", @"co_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCOFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCOFilterByURLModel alloc] init];
    }
    return _dataModel;
}
