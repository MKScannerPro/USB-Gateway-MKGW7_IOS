...
@implementation MKCOUserCredentialsViewModel
@end

@interface MKCOUserCredentialsView ()
...
@implementation MKCOUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCOUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCOUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(co_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate co_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(co_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate co_mqtt_userCredentials_passwordChanged:text];
            }
        };
