...
/**
 线程ID
 */
@property (nonatomic, assign)mk_co_taskOperationID operationID;
...
- (instancetype)initOperationWithID:(mk_co_taskOperationID)operationID
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKCOTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKCOTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_co_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_co_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    //分帧数据
    NSString *totalNum = returnData[mk_co_totalNumKey];
    if (MKValidStr(totalNum)) {
        //存在分帧数据
        self.receiveTimerCount = 0;
        NSData *data = returnData[mk_co_contentKey];
        if (data) {
